...
class SYWFields
{
            ...
            foreach ($field->value as $result) {
                if (!empty($options)) {
                    if (isset($options[$result]) && trim($options[$result]) !== '') {
                        if (JFactory::getLanguage()->hasKey($options[$result])) {
                            $cfield_values[] = JText::_($options[$result]);
                        } else {
    ...
    public static function prepareCustomFieldValue($context, $item, &$field)
    {
        ...
        $prepared_value = $dispatcher->trigger('onCustomFieldsPrepareField', array($context, $item, &$field));
        ...
        $dispatcher->trigger('onCustomFieldsAfterPrepareField', array($context, $item, $field, &$prepared_value));
