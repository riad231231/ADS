...
class SYWUtilities
{
	...
	static protected function getMobileDetector()
	{
		if (!isset(self::$mobile_detector)) {
			self::$mobile_detector = new SYW_Mobile_Detect;
		}
	...
	*/
	 ...
	 */
	static function getBootstrapProperty($property_string, $bootstrap_version = 2)
	{
...
// 		}
		...
		if (!empty($breakpoints)) {
            ...
            $html .= '<img src="' . ($fallback ? $source_path . '.' . $fallback_extension . $version : $src . $version) . '" alt="' . $alt . '" ' . trim(JArrayHelper::toString($attributes)) . '>';
